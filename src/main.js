...
const submitCreateButton = (e) => {
  e.preventDefault();
  if (
    checkName.test($createName.value) &&
    checkUserName.test($createPseudonym.value) &&
    checkPassword.test($createPassword.value)
  ) {
    replaceMenu();
    console.log('Działa');
  }
  ...
  if (checkUserName.test($createPseudonym.value) === false) {
    if ($unCorrectCreatePseudonym.innerText === '') {
      $unCorrectCreatePseudonym.innerText =
        'Dane nie są poprawne. \n\
  ...
  if (checkPassword.test($createPassword.value) === false) {
    if ($unCorrectCreatePassword.innerText === '') {
      $unCorrectCreatePassword.innerText +=
        'Dane nie są poprawne. \n\
...
const submitLoginButton = (e) => {
  e.preventDefault();
  if (
    checkName.test($loginPseudonym.value) &&
    checkPassword.test($loginPassword.value)
  ) {
    replaceMenu();
    console.log('Działa');
  }
  ...
  if (checkUserName.test($createPseudonym.value) === false) {
    $unCorrectLoginPseudonym.innerText +=
      'Dane nie są poprawne. \n\
            Pole Hasło musi zawierać conajmniej jedną cyfrę, dużą literę, jeden symbol (#?!@$%^&*-) \n\
            i nie może być krótsze niż 9 znaków';
  }
  ...
  if (checkPassword.test($createPassword.value) === false) {
    $unCorrectLoginPassword.innerText +=
      'Dane nie są poprawne. \n\
